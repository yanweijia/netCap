...
public class Tools {
	...
	public static String analyzeProtocol(Packet packet) {
		...
		if (protocolType.equals("TCP") || protocolType.equals("UDP") || protocolType.equals("ICMP")
			...
			protocolAnalyze += "Ttl:" + Integer.valueOf(hex.substring(44, 46), 16) + "\n";
			if (protocolType.equals("TCP")) {
				protocolAnalyze += "Protocol:TCP:" + Integer.valueOf(hex.substring(46, 48), 16) + "\n";
				protocolAnalyze += "Identififcation:" + Integer.valueOf(hex.substring(36, 40), 16) + "\n";
			} else if (protocolType.equals("UDP")) {
				protocolAnalyze += "Protocol:UDP:" + Integer.valueOf(hex.substring(46, 48), 16) + "\n";

				protocolAnalyze += "Header checksum：" + getHeaderChecksum(hex)+"\n";
			}
		...
		if (protocolType.equals("ARP")) {
			...
			protocolAnalyze += "Opcode:"
					+ (opcode == 1 ? "request" : (opcode == 2 ? "reply" : "unknow,code is" + opcode)) + "\n";
	 ...
	 * 
	 * @sum 相加之后的和
	 * @a 	用于取出第一位的数字时所需要的字符串
	 * @d 	经过计算的checksum
	 */

	public static Integer getHeaderChecksum(String hex) {
		String a;
		int d;
		int sum = 0 ;
		for (int i = 28; i < 64; i = i + 4) {
			
			sum = sum + Integer.valueOf(hex.substring(i, i + 4), 16);
			
		}

		a = Integer.toHexString(sum);		
		d = Integer.valueOf(a.substring(0, 1), 16);
		d = sum - d * 0x10000 + d;		
		d = 0xffff - d;
	...
	public static String hexMACAddressFormat(String hexMAC) {
		return hexMAC.substring(0, 2) + ":" + hexMAC.substring(2, 4) + ":" + hexMAC.substring(4, 6) + ":"
				+ hexMAC.substring(6, 8) + ":" + hexMAC.substring(8, 10) + ":" + hexMAC.substring(10);
	}
